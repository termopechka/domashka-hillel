...
def test_web_checkout_flow_success(auth_client, book_factory, mocker):
    ...
    order = Order.objects.get(user=user, city="Киев")
    assert order.payment_id == "cs_test_12345"

    mock_stripe.assert_called_once()
    assert mock_stripe.call_args.kwargs["metadata"] == {
