...
MIDDLEWARE = [
    ...
    "BookShop.middleware.RequestIDMiddleware",
...
WAREHOUSE_INTEGRATION_ENABLED = env_bool(
    "WAREHOUSE_INTEGRATION_ENABLED",
    default=False,
)
warehouse_hostport = os.environ.get("WAREHOUSE_HOSTPORT", "")
WAREHOUSE_BASE_URL = os.environ.get("WAREHOUSE_BASE_URL") or (
    f"http://{warehouse_hostport}/api/v1"
    if warehouse_hostport
    else "http://127.0.0.1:5000/api/v1"
)
WAREHOUSE_SERVICE_API_KEY = os.environ.get("WAREHOUSE_SERVICE_API_KEY", "")
WAREHOUSE_HTTP_TIMEOUT = (
    float(os.environ.get("WAREHOUSE_CONNECT_TIMEOUT", "2")),
    float(os.environ.get("WAREHOUSE_READ_TIMEOUT", "5")),
)
WAREHOUSE_HTTP_RETRIES = int(os.environ.get("WAREHOUSE_HTTP_RETRIES", "2"))
WAREHOUSE_HTTP_RETRY_BACKOFF = float(
    os.environ.get("WAREHOUSE_HTTP_RETRY_BACKOFF", "0.25")
)
...
LOGGING = {
    ...
    "filters": {
        "request_id": {
            "()": "BookShop.observability.RequestIDLogFilter",
        },
    },
    "handlers": {
        "console": {
